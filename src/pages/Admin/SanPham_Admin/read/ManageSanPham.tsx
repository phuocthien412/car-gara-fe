...
import { Package, PlusCircle, Edit3, Tag, Trash2 } from 'lucide-react'
...
export default function ManageSanPham() {
  ...
  return (
    <PageLayout>
      <PageHeader icon={Package} title="Danh sách Sản phẩm">
        <SearchBar
          value={q}
          onChange={setQ}
        ...
        />
        <ActionButton to={PATH.ADMIN_SAN_PHAM_CREATE} icon={PlusCircle} label="Thêm mới" />
      </PageHeader>
        ...
        {items.map((item, index) => (
          <CardWrapper key={item._id} index={index}>
            ...
            <div className="col-span-1 mt-3 sm:mt-0 flex items-center justify-end gap-2">
              <Link
                to={PATH.ADMIN_SAN_PHAM_UPDATE.replace('${item._id}', item._id)}
